...
| [3](part-01-loop/ch03-minimal-robotics-and-decision.md) | 像素、深度、关节与接触证据怎样进入同一 belief？ | 相机点、ego pose和车辆状态如何按各自时刻与有效位对齐？ | frame/unit/timestamp、异步多模态有效性、运动学与MDP/POMDP最小合同 |
...
能够稳定回答这三个问题，就已经完成本章的概念学习。实验和练习只是进一步检验理解的工具，不是阅读资格，也不是全书的中心。各章 fixture 相互独立，22 个 smoke 不能相加成“完整系统已经运行”；两个贯穿案例本身也没有成功率、模型大小或 GPU 结果，它们只提供稳定的问题身份，不能被解释为已经运行的机器人或车辆系统。
