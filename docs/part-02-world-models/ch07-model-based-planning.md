...
flowchart TB
    ...
    accDescr: 时刻 t 根据新观测规划 H 步；候选首步经独立安全网关核验后执行前缀，通常只执行首步；环境返回新观测后，未执行的旧后缀被废弃或仅作热启动，并从新状态重新规划 H 步。
    O0["时刻 $$t$$ 的新观测与 belief"] --> P0["规划 $$H$$ 步: $$a_t$$ 到 $$a_{t+H-1}$$"]
    P0 --> GW["独立安全网关核验"]
    GW --> X0["执行前缀: 通常只执行 $$a_t$$"]
...
*图 7-2：MPC 的滚动时域时间关系。`H` 是 prediction/planning horizon，实际连续执行的步数是 execution horizon；旧后缀不能绕过新观测自动取得执行权，待执行动作也必须先经独立安全网关。来源：本书原创，CC BY-NC 4.0，2026-09-02。*<!-- INTERNAL_ASSET_ID: FIG-07-02 -->
