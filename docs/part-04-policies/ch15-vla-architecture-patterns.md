...
视觉-语言-动作模型（Vision-Language-Action, VLA）怎样把视觉、语言和机器人状态变成可执行动作？离散 action token、连续回归、FAST、diffusion/flow action expert 与双系统架构的真正差别是什么？为什么一个会看图、会解释的视觉语言模型（Vision-Language Model, VLM）仍不能直接控制机器人或车辆？
...
读者应能从条件信息、主干、动作表示、时间组织和执行接口五条正交轴分析 VLA，解释语言语义如何逐层落到具体本体，并判断系统能力来自预训练知识、动作数据、控制器还是安全层。读者还应能区分 VLA 策略、通用 VLM、规划器与世界模型的证据边界。
...
VLA 描述的是输入语义与输出能力，不规定内部必须恰好有视觉编码器、语言模型和动作解码器三个独立模块。视觉与语言可以在早期融合，也可以由语言主干读取视觉 token；动作可以复用主干词表，也可以交给独立连续专家；本体状态既可以成为普通 token，也可以通过专门编码器注入。相同的 VLA 标签因此可能覆盖完全不同的信息路径与控制频率。

分析架构时更有用的是分开五条轴：模型看到了哪些条件，条件在哪里融合，动作采用何种表示，动作以什么时间结构产生，以及输出如何进入执行系统。逐维 token 与双系统不是互斥类别：前者回答动作怎样编码，后者回答计算怎样分层。同样，flow action head 也不自动意味着系统具有显式世界模型。

### 15.1.2 预训练知识只有经过动作数据才成为控制能力

大规模视觉语言预训练可以提供物体、属性、关系和任务常识，但它没有自然地规定某个机器人该用哪组关节速度完成动作。动作 grounding 需要把抽象语义连接到当前观测几何、本体可达性、控制模式和时间反馈。这一连接可以来自机器人示范、仿真数据、交互学习、传统规划器或人工接口，但必须有可审计来源。

因此，看到模型能正确回答“杯子在哪里”不能推出它能稳定抓杯子；看到它能输出格式合法的七维向量，也不能推出七个维度已绑定正确关节。VLA 的关键增量不是模型开始输出数字，而是这些输出在指定本体和闭环协议中获得了可验证含义。
...
### 15.2.1 Grounding 是一条层级链

一条语言指令通常要经过多个不同层级：指称 grounding 确定“杯子”对应哪个实体；空间 grounding 确定“左边”采用哪个参考系；任务 grounding 把“放置”展开为前置条件、阶段与终止标准；动作 grounding 再把当前阶段映射为本体可执行命令。任何一层含糊，都可能让后续动作在数值上合理而任务语义错误。

语言也不是永远有效的静态条件。对象被移走、任务被取消、操作者修正指令或安全状态改变后，旧语言计划可能失效。系统需要说明指令何时绑定观测、何时允许修订、冲突指令如何仲裁，以及低层 chunk 如何被撤销。把文本上下文无限保留并不等于保持任务一致性。

### 15.2.2 历史信息的价值取决于可辨识性

单帧图像可能无法判断物体速度、接触是否刚发生或夹爪正在打开还是闭合；历史与 proprioception 可以使隐藏状态更可辨识。但增加历史窗口不保证模型会使用它，也可能引入过期画面、时间错位和更高计算量。需要区分历史在输入中存在、表征中可读出，以及动作确实随相关历史反事实变化这三个证据层。
...
动作 token 虽然进入语言模型词表，却不因此获得自然语言语义。相邻 token ID 是否代表相邻动作、不同动作维以什么顺序生成、一个早期维度错误如何影响后续维度，都由 tokenizer 与序列化协议决定。若把本来同时发生的关节命令自回归展开，序列顺序还是一种建模偏置，而不是物理系统真的先执行第一维、再执行第二维。

量化还把连续误差改造成分类边界问题。两个物理上很近的动作可能落在不同 bin，两个同一 bin 内的动作则被视为相同；增大 bin 数可降低理论量化宽度，却会增加类别稀疏性和解码难度。判断 token 化是否合适，应看反归一化后的物理误差、时序长度与闭环后果，而不是只看 token accuracy。
...
慢层传给快层的内容决定了分层的信息瓶颈。如果只传一个离散技能名，接口稳定且易审计，但可能丢失目标姿态、约束和不确定性；如果传整段隐藏表示，信息更丰富，却难以解释其时效、版本与失效条件。理想接口不一定最宽，而是包含快层完成当前控制所需的充分信息，同时具有明确语义和生命周期。

快慢频率也不是模型属性，而是任务与部署共同定义的时标。语义目标可以低频刷新，但接触切换、动态障碍和急停信号不能等待慢层周期。一个合理的分层系统会为不同信息设置不同更新通道：任务意图可以缓存，状态反馈持续进入快层，安全事件则绕过两者获得最高优先级。
...
“迁移到新机器人”可能只复用视觉语言表征，也可能复用任务分解、动作先验或完整策略。越靠近动作端，迁移越依赖几何、动力学和控制接口是否对齐。共享“抓取”概念不意味着共享末端轨迹，共享末端轨迹也不意味着共享关节命令；后两者还分别需要坐标变换、逆运动学与低层控制适配。

本体条件 token 可以告诉模型当前使用哪种机器人，却不能凭名称解决未见过的运动学。若训练数据中本体标签与任务或场景高度相关，模型甚至可能把标签当作数据集捷径，而没有学到可组合的本体语义。跨本体能力应按对象语义、任务阶段、末端行为和底层动作逐层评估，明确哪一层真正复用、哪一层重新学习。

## 15.8 自动驾驶：语言辅助决策，不直接接管控制
...
VLA 是视觉、语言和本体条件下的动作策略族，不是一种固定网络拼接。预训练知识只有经过指称、空间、任务与动作 grounding，才可能成为指定机器人的控制能力。条件融合、动作表示、时间组织和执行接口是不同架构轴：离散 token、FAST、连续回归和 diffusion/flow expert 是动作表示与生成方式，双系统则是计算和信息流的组织方式。

动作 token 不是天然带语义的语言，慢层也不是天然可靠的规划器；跨本体 token 更不能替代运动学与控制适配。所有路线最终都必须落到本体状态、解码、归一化、frame、时间、控制器和安全层，并依据可查询行为而非模块名称判断 VLA、世界模型与规划器的边界。
