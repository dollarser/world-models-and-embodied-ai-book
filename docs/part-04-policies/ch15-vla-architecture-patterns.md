...
第16章将处理跨数据集/本体的动作对齐、数据混合、LoRA/OFT、蒸馏和异步执行；第17章再判断世界模型究竟怎样帮助 VLA，而不是把两者名称直接拼接。
...
- 内容审查：修改中；
- 代码审查：修改中；
- 一致性审查：修改中（已与第10/12/13/14章对齐，等待第16/17/20章）；
- 教学审查：修改中；
- 审查记录路径：待批次 B/C 交叉审查；
- 已知限制：没有下载或运行任何 VLA、VLM API、机器人、仿真或 GPU；
- 下一步：完成第16章数据/本体适配后，对第13–16章做批次 B 交叉审查。
