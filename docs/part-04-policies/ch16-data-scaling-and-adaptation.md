...
读者应能区分数据体量、有效信息量与目标分布覆盖，解释 mixture 权重如何定义学习目标，并判断 canonical action 对哪些本体差异保持不变、丢失了什么。读者还应能从共享结构、梯度冲突和适配权限理解正负迁移，而不是把参数规模或数据总量直接当成迁移能力。
...
相邻视频帧、同一轨迹的重叠窗口和同一示范的增广副本具有强相关性。它们会增加 transition 计数，却不会按相同比例增加独立信息。重复记录有时能帮助学习稳定的低层规律，但不能替代新任务、新失败模式或新交互分支。因而“样本数翻倍”只有在抽样单位、相关结构和新增覆盖明确时才有可解释意义。

规模化至少可能带来三类收益：更密地覆盖已有状态附近的变化，更广地覆盖任务和环境，或引入可跨任务复用的结构。三者对应的失败也不同。密度增加可能很快饱和，广度增加可能产生互相冲突的标签，而共享结构不足时，更多异质数据只会增加优化负担。数据曲线应按来源和能力维度解释，不应只画总 token 对总体分数。

有效数据还由策略目标决定。大量稳定搬运示范可能对常规操作有用，却几乎不包含恢复、失败和安全边界；稀有恢复片段虽然数量少，对闭环占用分布却可能更关键。因此，数据价值不是来源固有常数，而是相对于目标任务、损失和部署风险定义的。

### 16.1.2 Mixture 权重在声明“我们想学谁”

加权目标不是中性的装载细节。选择 $w_d$ 等于规定各来源在优化目标中的相对重要性，也隐含了对未来部署分布的判断。dataset-uniform 强调来源平衡，transition-uniform 强调记录量，task-balanced 或 risk-aware 采样则强调任务与后果；不存在脱离目标场景的普遍正确权重。
...
### 16.3.1 Canonical 表示是一种归纳偏置

把不同本体映射到共同动作空间，相当于声明某些物理差异对当前任务不重要。例如末端位姿增量忽略了具体关节如何配合，只保留工具在空间中的运动；技能 token 进一步忽略连续轨迹，只保留任务语义。这种不变性可能促进迁移，也可能丢掉避开奇异位形、满足力限制或利用本体特性的关键信息。

因此 canonical action 不是越抽象越好，而是在共享性与可执行性之间选择接口。低层表示保留控制细节但难跨本体，高层表示容易共享却需要更强的本体专用解码器。合理设计应回答两个问题：哪些差异希望共享模型忽略，哪些差异必须通过 embodiment condition 或 decoder 恢复。

转换还可能是多对一或一对多。多个关节构型可以产生同一末端位姿，同一高层技能也可以由多条轨迹实现；此时 raw→canonical→raw 不可能无条件唯一。round-trip 只适合检查声明为可逆的字段。对有损接口，更诚实的合同是验证 canonical 语义保持、目标本体可达性和闭环任务效果，并记录被消去的信息。
...
归一化还改变了优化问题中的距离几何。若每维都标准化到相似方差，低变化但安全关键的夹爪或制动维度可能被放大，高变化关节则被压缩；若完全不标准化，大量程维度又可能主导损失。所谓“各维公平”必须相对于任务后果定义，统计尺度相同不代表物理重要性相同。
...
<!-- CLAIM_META: CLAIM-16-04 result -->
fixture 对缺失/未知 `embodiment_id`、缺失 fingerprint 和陈旧 fingerprint 的 3 条错误记录全部拒绝转换，而不是套用默认本体或当前 adapter；这只验证 metadata 与版本门禁。

<!-- CLAIM_META: CLAIM-16-07 result -->
fixture 中缩放或字段合同改变会产生不同 schema fingerprint；该结果只证明确定性身份绑定，不提供防篡改、数据真实性、跨语言序列化兼容或 controller 安全保证。
...
<!-- CLAIM_META: CLAIM-16-05 recommendation -->
跨本体训练应以“目标单独训练”为基线，用固定预算的来源×目标迁移矩阵报告正/负迁移；只有混合模型分数或总体平均无法定位贡献。

### 16.6.1 迁移来自什么

正迁移可能来自共享视觉概念、相似任务阶段、共同末端运动、更多语言变化，或对优化过程的正则化；负迁移则可能来自动作语义冲突、条件不足、模型容量竞争、采样失衡或来源捷径。看到加入来源数据后目标性能变化，只能确认该训练协议下存在净影响，不能自动定位机制。

机制诊断需要有针对性的对照。例如保留图像和语言、替换动作标签，可以检查收益是否依赖动作结构；保留来源数据但提供或移除 embodiment tag，可以检查冲突是否可被条件化消解；使用共享主干加专用 head，可以区分表征共享与动作共享。对照应一次隔离一条假设，而不是同时改变数据量、模型容量和训练步数。

固定训练更新与固定目标数据曝光回答不同问题。前者比较相同计算预算下 mixture 是否值得，加入来源数据会挤占目标样本；后者保持目标学习机会不变，但总计算与总数据增加。两种结论都可成立，却不能共用“数据带来提升”的一句话。迁移矩阵必须注明采用 compute-matched、target-exposure-matched 还是 data-added 协议。

### 16.6.2 把“未见本体”变成受控隔离变量
...
<!-- CLAIM_META: CLAIM-16-08 fact -->
XEWorld v1 在五种双臂机器人和 25 个操作任务上定义了 held-out-embodiment 协议，并把视觉质量、机器人形态、运动学与物体动力学分开报告；这一事实只描述论文协议，不证明其作者结论已由本书复现或适用于所有世界模型。
...
### 16.7.1 适配权限应匹配需要改变的能力

选择适配方法前，先判断失配发生在哪一层。若只是动作字段和控制器变化，schema adapter 或新 head 更直接；若视觉外观变化但任务语义稳定，需要改变视觉表征；若新指令要求新的概念组合，语言与跨模态层可能需要更新；若模型根本缺少目标本体的动力学经验，只增加 LoRA rank 也不会凭空提供监督。

参数高效只描述有多少参数参与更新，不保证学习问题更简单。低秩更新假设所需能力变化可以由受限子空间表达，这在相近域上可能有效，在需要重写底层几何或控制语义时可能不足。反过来，full fine-tune 提供更大自由度，也更容易破坏已有能力。权限阶梯的意义是用最小充分修改验证假设，而不是预设越小或越大越先进。

蒸馏则改变了知识来源与可见行为。student 只能学习 teacher 在蒸馏数据覆盖状态上展现的输出；若 teacher 的有用能力只在特定提示、罕见恢复状态或候选分布尾部出现，简单点预测蒸馏会把它们抹去。teacher 更大也不构成正确性保证，student 还可能更忠实地复制其系统偏差。因此应分别讨论压缩率、行为覆盖、分布保持和闭环后果。
...
<!-- CLAIM_META: CLAIM-16-06 recommendation -->
自动驾驶跨车队训练不得直接混合同名 raw control；应转换到版本化轨迹/动力学合同或使用 fleet-specific adapter，并以按车辆/路线拆分的闭环迁移矩阵验证。

驾驶数据还包含人脸、车牌、地理位置和行为记录，必须处理授权、脱敏、保留期限和跨地域治理。格式转换、裁剪和衍生 embedding 不会消除源数据许可与隐私责任。

## 16.10 资源、数据和许可路线

全书资源档位见[术语表](../glossary.md)。本章的最低反例只验证 adapter、normalization、mixture 暴露和 action-window 分母；进一步使用真实数据时，应先完成 schema round-trip、split/来源身份、数据 checksum、许可和实际有效窗口统计，再讨论 adapter、LoRA/OFT 或蒸馏。模型适配不能补救错误的数据合同。

大型数据和 checkpoint 不应写进镜像层或由正文命令自动下载。使用前必须登记文件清单、预计字节数、缓存位置、终止方式，以及代码、模型、每个子数据集、视频和衍生资产各自的许可。
...
数据规模化的核心不是拼文件，也不是累计 transition，而是增加与目标任务相关的独立信息和覆盖。Mixture 权重实际定义了学习目标，采样暴露、loss 权重与梯度影响又是不同层次。canonical action 是选择忽略哪些本体差异的归纳偏置：越抽象越容易共享，也越依赖专用解码器恢复可执行细节；有损映射不能用机械 round-trip 冒充语义保持。

正负迁移是共享结构、条件信息、容量竞争和训练预算共同作用的净结果，需要迁移矩阵与机制对照解释。归一化改变优化几何，LoRA/OFT、部分解冻和 full fine-tune 提供不同修改权限，蒸馏则受 teacher 行为覆盖约束。所有结论都必须注明 seen、few-shot 或 zero-shot 的目标本体，以及 compute-matched、target-exposure-matched 或 data-added 的比较口径。
...
等数据集权重忽略 episode 数量，三者均为 `1/3`，再在各数据集内部采 episode。温度方案可预注册 $w_i\propto n_i^{\alpha}$；取 $\alpha=0.5$（等价于 size temperature $T=2$）时，`sqrt(100):sqrt(1000):sqrt(10000)=10:31.62:100`，归一化约为 `0.0706,0.2233,0.7061`。作对照，$\alpha=1$ 的按规模权重约为 `0.0090,0.0901,0.9009`，$\alpha=0$ 回到等数据集。还应报告有效步/任务权重和重复率；episode 数不同不等于质量或多样性不同。
