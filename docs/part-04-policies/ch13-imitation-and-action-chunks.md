...
`CLAIM-13-02`（result）：在 `EXP-13-01` 中，每步 `0.02` 的动作偏差在 20 步后形成 `0.40` 的状态偏差；固定预测 horizon 为 8、将执行 horizon 从 1 增到 8，使 policy query 从 16 次降到 2 次，同时最大陈旧动作等待从 0 增到 7 步。该结果只展示固定标量规则下的调用—陈旧权衡，不估计学习策略的误差或真实时延。
