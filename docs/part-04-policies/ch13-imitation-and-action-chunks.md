...
*FIG-13-01：模仿策略的闭环接口。虚线表示专家数据只约束其访问过的状态；安全层不属于策略权重。来源：本书原创，CC BY-NC 4.0，2026-08-31。*
...
```mermaid
flowchart TB
    accTitle: FIG-13-02 动作块执行与时间集成的两种时域协议
...
*FIG-13-02：动作块的两种执行协议。左路用较少查询换取更长开环承诺，右路用逐步查询形成可集成的重叠预测；两者都必须响应事件触发的失效条件。来源：本书原创，CC BY-NC 4.0，2026-09-02。*
...
`EXP-13-01` v4 的两个手写策略在唯一专家支持点 `(0,0)` 上 action MSE 同为0；同受 `x_0=0.25` 的 reset disturbance 后，六步最终绝对状态分别为0.00390625和2.84765625。该固定反例只证明 expert-support loss 不识别 off-support feedback behavior，不估计 learned policy 的泛化、DAgger 收益、真实扰动概率、稳定域或安全性。
...
在 `EXP-13-01` 中，持续同号的每步 `0.02` 动作偏差在 20 步单位增益积分后形成 `0.40` 的状态偏差；固定预测 horizon 为 8、将执行 horizon 从 1 增到 8，使 policy query 从 16 次降到 2 次，同时最大陈旧动作等待从 0 增到 7 步。前者不是闭环策略评测，后者只展示固定周期执行规则下的调用—陈旧权衡；两者都不估计学习策略误差或真实时延。
...
`EXP-13-01` v4 中，持续同号与正负交替误差的动作 RMSE/MAE 都是 `0.02`，但 20 步后的积分状态误差为 `0.40/0`，交替序列的最大瞬态状态误差仍为 `0.02`。该负对照只证明逐点幅值指标丢失误差符号的时间相关性，不能推出真实反馈系统中哪种策略更安全或误差会线性累积。
...
`EXP-13-01` v4 中，时间集成把固定稳态预测 `[0.8,1.2,0.8,1.2]` 对 target 1 的误差从 0.2 降到约 0.001；但当预测为 `[0,0,0,1]` 且真实 target 已变为 1 时，ensemble 仍为约 0.246，误差约 0.754。它证明 smoothing 与 change-response 必须分别测试，不估计真实策略发生频率。
