...
行为克隆是必要但不充分的基线：开环动作误差测的是专家分布上的拟合，闭环评测才观察策略制造的输入分布；即使开环 RMSE 相同，误差的方向与时间相关性也可能改变状态后果。动作分块联合预测局部动作，只有 `K_exec>1` 才以更少查询换取潜在反应延迟，而 temporal ensembling 通常依赖逐步查询；它们必须与控制频率、重规划、安全层一起定义。
