...
行为克隆优化的是专家占用分布上的逐点预测风险。若用 $d_{\text{expert}}$ 表示专家在状态空间中的访问频率，它关心的是

\[
R_{\text{BC}}(\pi)=\mathbb{E}_{s\sim d_E,\,a^*\sim\pi_E(\cdot\mid s)}[\ell(\pi(s),a^*)].
\]

部署真正关心的却通常是策略自身占用分布 $d_{\pi}$ 下的累计任务代价。$d_{\pi}$ 不是一个固定测试集：策略动作经由环境转移改变下一状态，下一状态又改变后续动作。因此，即使模型参数只发生很小变化，访问到的状态、失败类型和任务损失也可能发生非线性变化。这里存在两层差异：同一状态上动作预测是否正确，以及策略是否仍会访问专家曾覆盖的状态。开环验证主要回答前者，闭环 rollout 才同时暴露后者。
...
flowchart TB
    ...
    Q["当前观测触发一次 $$K_{\text{pred}}$$ 步预测"] --> M{选择执行协议}
    M --> C["块执行: $$K_{\text{exec}}$$ 大于 1"]
    C --> E["连续执行前 $$K_{\text{exec}}$$ 步"]
    E --> N[吸收新观测并重新查询]
    N --> D[旧块剩余后缀失效]
    M --> T["时间集成: $$K_{\text{exec}}$$ 等于 1"]
    T --> R[每个控制步重新查询]
...
端到端反应时间至少包含感知数据年龄、预处理与策略推理、通信排队、执行器响应，以及等待下一次允许重规划的时间。动作块只控制其中最后一部分，却可能与其他部分叠加。因而安全问题不是“$K_{\text{exec}}$ 是否足够小”，而是最坏情况下的新证据要经过多久才能改变车辆实际控制。周期性重规划给出时间上界，事件触发中断则为切入、障碍物或定位异常提供更快通道；两者都失效时，独立安全层必须能够覆盖学习策略。
...
动作分块联合预测局部动作，本质上把预测范围与重新吸收反馈的频率拆开。只有 $K_{\text{exec}}>1$ 才以更少查询换取潜在反应延迟，temporal ensembling 通常依赖逐步查询；动作序列也不自动成为具有终止语义的高层技能。部署协议必须同时定义 $K_{\text{pred}}$、$K_{\text{exec}}$、控制频率、端到端时延、缓存失效和独立安全层。
