...
| 批次 C（部分通过）：第3、10、11、12、19章 | `drafted` | 第3、12、19章四类审查通过；五章共 32 个单元测试与零下载 CPU smoke | 第10、11章等待第5章生成式基础一致性审查；真实 3D/仿真/GPU 未运行 |
...
| 第10章 非生成式预测表示 | `drafted` | 内容、代码、教学审查通过；JEPA 谱系、probe 协议与自动驾驶状态读出 | 等待第5章一致性；官方 checkpoint、真实数据与 GPU 未运行 |
| EXP-10-01 | `smoke` | 5 个单元测试；重建与 shifted probe 排名反转 | 手工标量表征，不是 JEPA 性能 |
| 第11章 动作条件视频世界模型 | `drafted` | 内容、代码、教学审查通过；动作反事实与 renderer/simulator/planner 边界 | 等待第5章一致性；视频训练、checkpoint、仿真和 GPU 未运行 |
