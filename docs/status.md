...
| 批次 B（进行中）：第13、14、20章 | `drafted` | 三章正文、15 个单元测试与零下载 CPU smoke | 等待第15/17/19章后做一致性审查；上游策略/仿真未运行 |
...
| 第14章 生成动作 | `drafted` | 多峰动作、diffusion/flow 最小桥接、采样预算与自动驾驶安全筛选 | Push-T/LIBERO、学习策略、GPU 和真实时延均未运行 |
...
| 文档站 | `smoke` | 11 章正文接入 MkDocs Material Docker 严格构建 | 发布部署与浏览器视觉审查 |
