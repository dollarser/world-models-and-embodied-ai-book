...
```mermaid
flowchart TB
    accTitle: FIG-17-01 图 17-1 世界模型帮助策略的五条路径
...
*图 17-1：世界模型帮助策略的五条路径。来源：本书原创，CC BY-NC 4.0，2026-09-01。五类是数据流角色，不是互斥算法标签；一个系统可以同时跨多条路径。*<!-- INTERNAL_ASSET_ID: FIG-17-01 -->
...
<!-- CLAIM_META: CLAIM-17-01 fact -->
五类用途共享预测模型，却不共享完成标准；表征 probe、生成质量、交互稳定性、规划 return 和安全漏检率不能互相替代。
...
<!-- CLAIM_META: CLAIM-17-09 fact -->
A2World 论文把同一个 action-to-video 预训练先验分别适配为 A2World-sim 与 A2World-policy，而官方仓库快照 `077e10a` 明确把当前代码发布限定在 world-model/A2World-sim 侧；这一事实只描述论文架构和已发布资产范围，不证明两种能力已由本书复现、同等开放或可用同一指标验收。
...
<!-- CLAIM_META: CLAIM-17-03 recommendation -->
用于规划或安全筛选的世界模型必须在候选策略诱导分布上报告动作条件转移、return/风险 gap、OOD 与漏检；高平均预测准确率不能单独授权执行。
...
## 17.7 风险旁路：把世界模型当评测替身

[WorldEval](https://arxiv.org/abs/2505.19017) 和 [WorldGym](https://arxiv.org/abs/2506.00613) 研究用视频世界模型运行或排序机器人策略；WorldGym 的官方仓库提供 OpenVLA、Octo、SpatialVLA 和 RT-1-X runner `[A/O,R1]`。这是一条重要的降本路线，但它不向策略提供训练数据、表征、交互环境、规划信息或安全动作，而是在真实评测之外提供代理结果。因此本书把它画在五条主路径之外，称为风险旁路，而不是含混地称作“第六种帮助方式”。

<!-- CLAIM_META: CLAIM-17-04 inference -->
世界模型中的策略排序与真实排序相关，只支持其已验证任务、策略族和协议中的筛选用途；相关性不能校准绝对成功率，也不能替代新策略、OOD 场景和最终安全评测。
...
*表 17-1：代理世界模型评测的四段归因账。来源：本书原创接口；实验 17-1 v6 只验证单故障机制。*<!-- INTERNAL_ASSET_ID: TAB-17-01 -->
...
实验 17-1 v6<!-- INTERNAL_ASSET_ID: EXP-17-01 v6 --> 给四段各注入一个确定性故障。全正确 oracle 保持 `safe_route`、Spearman 为 `1`、regret 为 `0`；action-grounding 故障把安全策略错误注入为 shortcut，代理改选 `idle`，真实 regret 为 `1.05`。更关键的是，transition 把碰撞预测成 goal、decoder 把 collision 解码成 goal、scorer 把 collision 直接打成 `1.0` 时，三者产生完全相同的最终三策略分数、都选择 `phantom_shortcut`，Spearman 都为 `-0.5`、真实 regret 都为 `1.85`。只有检查中间的 predicted terminal 与 decoded terminal 才能定位故障段。

<!-- CLAIM_META: CLAIM-17-10 result -->
在 实验 17-1 v6<!-- INTERNAL_ASSET_ID: EXP-17-01 v6 --> 的单故障四段管线中，transition、state decoder 和 outcome scorer 三个不同故障产生相同最终代理分数与错误策略选择；因此端到端相关性、成功分或 regret 不能单独归因到某个组件。该结果只证明固定 corridor 中的不可辨识反例，不给出真实组件故障率、误差独立性或可加总的统计预算。
...
## 17.8 8/9 正确仍选中碰撞策略（实验 17-1<!-- INTERNAL_ASSET_ID: EXP-17-01 -->）
...
*表 17-3：实验 17-1 的模型 gap 与策略排序。固定规则用于说明接口，不是 learned simulator benchmark。*<!-- INTERNAL_ASSET_ID: TAB-17-03 -->

<!-- CLAIM_META: CLAIM-17-02 result -->
实验 17-1<!-- INTERNAL_ASSET_ID: EXP-17-01 --> 的学习模型单步一致率为 `8/9`，却把真实最优 `safe_route` 排在乐观捷径之后；模型所选策略在真实规则中碰撞，排序 Spearman 为 `-0.5`，exploitation regret 为 `1.85`。该结果只验证手工 corridor 中的模型利用机制，不估计学习式世界模型的真实碰撞率。
...
<!-- CLAIM_META: CLAIM-17-08 result -->
保持世界模型、策略和真实规则不变，只把 `(position=0, shortcut)` 加入手工 support 后，gate 接受全部三个策略，仍选中真实会碰撞的 `phantom_shortcut`，exploitation regret 保持 `1.85`。该负对照只证明 coverage membership 不能发现 support 内模型错误，也不估计真实数据覆盖质量或 learned uncertainty 的失效率。
...
*表 17-5：用途决定证据。一个系统可跨多行，但必须分别报告。*<!-- INTERNAL_ASSET_ID: TAB-17-05 -->
...
**杯子任务。** 同一个世界模型可以为杯子与夹爪提供预训练表征、生成遮挡与滑落样本、充当策略学习的交互环境、为候选抓取提供 rollout 与价值，或在执行前预测碰撞和掉落风险；这五种用途需要不同证据。若它只替代真实环境给策略打分，则属于更高风险的代理评测角色：模型可能偏爱自己最容易预测的抓取方式，并把这种偏好误写成策略进步。每条路径都要回到未参与训练和选择的对象、初态与真实结果上校准，不能因为共享一个模型名称就合并验收。

## 17.10 自动驾驶：四个角色，四套证据

自动驾驶世界模型可承担：生成雨夜/切入场景，按候选转向与制动 rollout，给规划器估计碰撞/舒适代价，以及对稀有事件做反事实压力测试。四者不能共享一个“视频质量很好”的结论。

一个驾驶 planner 若发现世界模型中的“穿过护栏捷径”，会像 实验 17-1<!-- INTERNAL_ASSET_ID: EXP-17-01 --> 一样主动利用盲区。评测必须按道路使用者、碰撞严重度、车速、遮挡、地图/坐标误差和动作范围分桶；对模型所选轨迹，再由车辆动力学、道路边界、occupancy、控制限幅和最小风险层独立检查。
...
<!-- CLAIM_META: CLAIM-17-05 recommendation -->
自动驾驶使用世界模型时，应为场景生成、驾驶 rollout、规划代价和稀有事件验证分别登记数据、horizon、动作合同和真实性锚点；任何一类通过都不能授权另一类用途。

<!-- CLAIM_META: CLAIM-17-06 recommendation -->
驾驶与机器人数据扩充应优先审查当前策略可达、模型过度乐观且后果严重的状态，而不是只按平均预测误差排序；选择协议需要同时登记可达性、不确定性、风险与覆盖成本。

## 17.11 资源、开源与许可路线

全书资源档位采用[术语表](../glossary.md)中的统一定义。本章的升级顺序应跟随证据链：S 档先用解析反例定位模型错误如何影响策略；随后才在轻量物理仿真中比较 held-out transition、return gap 和策略排序；只有这些接口成立后，运行 TD-MPC2、V-JEPA 2-AC 或 WorldGym 一类系统才有解释价值。代理评测还需要 policy runner、动作转换、world model 和 outcome scorer，单个 checkpoint 的大小不能代表整条链路成本。

这些扩展当前均不构成本书结果。V-JEPA 2 的不同 encoder 与 action-conditioned checkpoint 不能互换控制证据；WorldGym/WorldEval 和 Cosmos 等大型路线超出资源上限时保留为论文与接口案例，不要求读者运行。
...
| 类型 | 声明/结果 | 来源 | 状态 | 限制 |
| --- | --- | --- | --- | --- |
| 本书结果 | 8/9 转移一致但策略错排；gate 阻断 support 外捷径、却不能阻断同一 support 内错误 | 实验 17-1<!-- INTERNAL_ASSET_ID: EXP-17-01 --> | CPU smoke | 手工 corridor 与两套 authored support 声明 |
...
先独立写清用途、选择准则和真实性锚点，再展开自检。这里的数值只对应 实验 17-1<!-- INTERNAL_ASSET_ID: EXP-17-01 --> 或题目中明确给出的反事实，不是 learned world model 的实测性能。

<details markdown="1">
<summary>自检 17-1：冻结 encoder 不等于在线 simulator</summary>
...
<details markdown="1">
<summary>自检 17-2：随机乐观错误要同时看均值与尾部</summary>
...
<details markdown="1">
<summary>自检 17-3：receding horizon 不能撤回错误首步</summary>
...
<details markdown="1">
<summary>自检 17-4：代理评测的预注册合同</summary>
...
<details markdown="1">
<summary>自检 17-5：施工锥幻觉与最小风险动作</summary>
