...
- 22 张实验卡、22 组零下载或微型 CPU fixture、结构化结果与 Docker smoke；
...
这些命令验证共 142 个章节单元测试、标准库 fixture、输入合同、22 组 smoke—结果 JSON 精确一致性、实验卡/manifest Schema、源码链接、导航、严格站点构建和生成站点内部资源。当前站点还提供面向读者的可检索术语表。每章都完成内容、代码、一致性和教学审查，机器可读状态为 `reviewed`。
...
- 已用回环静态服务器打开严格编译产物的应用内浏览器面板，并自动检查 26 个 HTML、22 章页面和 877 个内部目标；截图式多尺寸与可访问性巡检仍需在发布操作中人工确认；
