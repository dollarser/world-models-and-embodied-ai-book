...
这些命令验证共 136 个单元测试、标准库 fixture、输入合同、22 组 smoke—结果 JSON 精确一致性、实验卡/manifest Schema、本地链接、导航和严格站点构建。每章都完成内容、代码、一致性和教学审查，机器可读状态为 `reviewed`。
...
- 发布操作仍需选择托管平台并做浏览器、多尺寸与可访问性视觉抽查；本次本地站点可启动，但当前主机的应用内浏览器运行时不可用，未伪报视觉验收；
