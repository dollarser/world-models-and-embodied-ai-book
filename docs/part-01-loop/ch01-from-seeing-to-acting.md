...
两条序列都由五个 `-0.1` 与三个 `+0.1` 组成，平均绝对扰动都是0.1；在同一 $k=0.8$、两步时延和 $u_{\max}=0.25$ 下，最终 state 也都为0.264。第一条轨迹最远只到 `-0.3`，而本章严格边界要求 $|x|>0.3$，所以不越界；第二条在第7步到0.476，已经产生不能由最终0.264抹去的事件。合格答案应同时报告 trace/峰值/首次越界，而不是只比较终态；也必须说明这仍是有限标量反例，不是稳定性或真实安全证明。
