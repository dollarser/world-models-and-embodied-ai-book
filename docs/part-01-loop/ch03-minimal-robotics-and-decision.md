...
马尔可夫决策过程（MDP）用状态、动作、转移、奖励和折扣描述序贯决策。若真实状态 $e_t$ 不可完全观测，系统只得到 $o_t$，问题更接近部分可观测 MDP（POMDP）。策略可以基于历史或信念状态：真实状态 $e_t$ 经传感器产生观测 $o_t$，历史 $(o_{\le t},a_{<t})$ 经估计/记忆形成信念 $z_t$，再由策略 $\pi(a_t\mid z_t)$ 执行并改变环境。
