...
最小自检是 round trip：把反投影点再投影回像素，误差应接近数值精度。如果 round trip 通过但点云尺度错 1000 倍，通常是毫米/米错误；如果中心附近正确、边缘系统性弯曲，可能是畸变未处理；如果 RGB 与深度边缘错位，要检查配准与时间同步。
...
3. **几何练习**：将 body 点变回 camera，验证正逆变换 round trip。
...
- 下一步：用本章 schema 复核第4、12、13、19章接口，并补充时间错位 fixture。
