...
| --- | --- | --- | --- |
| 实验分组 | `group_id` | 协议声明的 route/scene/episode 被拆开 | 不同 ID 背后是否同源 |
| 原始来源 | `source_asset_id` | 两个派生片段来自同一原始 log/video/session | 同一路线的不同采集是否相似 |
...
驾驶数据具有更强的空间与时间重复性。连续数千帧共享道路纹理、标志、天气、相机标定和交通参与者。即使按日志文件随机切分，同一路线的相邻行程也可能导致泄漏；同一 raw log 切出的多个文件即使拥有不同文件名、episode ID 和 route ID，也仍是直接同源样本。
...
- **身份别名泄漏**：重命名或重新编号掩盖同一 raw asset、精确内容或已知近重复簇；
- **时间错位**：动作与错误的下一观测配对；
- **统计泄漏**：用完整数据拟合归一化或词表；
