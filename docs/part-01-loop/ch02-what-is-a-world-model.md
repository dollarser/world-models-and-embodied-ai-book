...
*表 2-1：实验 2-1 v4 的等权 state-aliasing fixture。return 是无单位教学效用，不是碰撞代价、驾驶 reward 或概率估计。*<!-- INTERNAL_ASSET_ID: TAB-02-01 -->
...
在 实验 2-1<!-- INTERNAL_ASSET_ID: EXP-02-01 --> 的两个等权手工 context 中，current-only 表示把不同最优动作的历史合并，最优共享动作仍有 0.5 mean regret；保留历史线索后固定 regret 为 0。该差值只证明这组一次决策 fixture 是 memory-improvable，不证明某种 sequence model 必然能学会记忆，也不估计真实 POMDP、机器人或驾驶性能。
...
较新的 [POBAX 论文](https://arxiv.org/abs/2508.00046) 与[锁定源码快照 `a5e1d62`](https://github.com/taodav/pobax/tree/a5e1d62d14e4efe783885b9d4f19cffa2a568eec)把“更多 state 信息或 memory 能否形成清晰 performance gap”作为部分可观测 benchmark 的设计信号，并覆盖多种 state aliasing。它适合作为需要 JAX 与策略训练的可选 L1 档证据路径，不是理解本章概念所依赖的 S 档内容；本书没有运行 JAX、训练 memory policy 或复现其论文结果。
...
```mermaid
flowchart TB
    accTitle: FIG-02-01 图 2-1 世界模型四轴证据卡
...
*图 2-1：四轴模型卡把名称拆成表示、推进、条件和用途，再由证据决定允许发布的能力声明。来源：本书原创，CC BY-NC 4.0，2026-08-31。*<!-- INTERNAL_ASSET_ID: FIG-02-01 -->
...
表 2-3<!-- INTERNAL_ASSET_ID: TAB-02-03 --> 给出系统族的分类示例。它不是排行榜，也不把某个产品版本的全部能力固定下来。
...
下面的矩阵来自 实验 2-1 v4<!-- INTERNAL_ASSET_ID: EXP-02-01 v4 --> 的八张固定卡。它刻意拆开四个经常被错误合并的命题：有时间/转移预测、能输入候选动作、动态是学习得到的、以及直接输出策略却没有独立转移接口。
...
*表 2-3：实验 2-1 v4 的能力证据矩阵。状态描述锁定卡片，而不是对同名系统未来版本或整个研究方向作永久判断。*<!-- INTERNAL_ASSET_ID: TAB-02-03 -->
...
### 2.4.1 同名异义：世界模型的六条技术谱系

“世界模型”并不对应一种固定网络结构。它更像一个**功能性总称**：模型从历史信息中形成某种内部状态，并对尚未观测的状态、表征、回报或观测作出预测，使智能体能够理解变化、比较行动后果，或者构造可供训练与评估的环境。不同工作对“内部状态”“未来”和“有用”的定义不同，于是会得到看起来差异很大的方案。

现代语境中的分歧并非谁误用了术语，而是几条研究传统在同一个名称下汇合：控制领域关心模型能否支持规划，强化学习关心能否在想象轨迹中学习策略，表征学习关心能否捕获可预测的抽象结构，生成模型关心能否产生连续且可控的观测，机器人与自动驾驶团队还会进一步关心数据生产、闭环评估和安全验证。阅读论文或产品说明时，不能只问“它是不是世界模型”，还要问“它把世界模型放在闭环的哪个位置”。

```mermaid
flowchart TB
    accTitle: FIG-02-02 图 2-2 世界模型问题谱系
    accDescr: 环境建模与系统辨识分叉为潜在动力学、预测表征与生成式未来预测三条路线，各自通向具身智能闭环；用于区分问题定义，不是代际排名。
  A["环境建模与系统辨识<br/>状态转移、观测与不确定性"] --> B["潜在动力学<br/>World Models、PlaNet"]
  B --> C1["想象中的策略学习<br/>Dreamer 系列"]
  B --> C2["面向搜索的决策模型<br/>MuZero 等"]
  A --> D["预测表征<br/>JEPA、V-JEPA"]
  D --> E["动作条件表征预测<br/>V-JEPA 2-AC 等"]
  A --> F["生成式未来预测<br/>视频、token 或多模态状态"]
  F --> G1["可交互环境<br/>Genie 系列"]
  F --> G2["垂直领域仿真与评估<br/>GAIA 等"]
  F --> G3["世界基础模型平台<br/>Cosmos 等"]
  C1 --> H["具身智能闭环"]
  C2 --> H
  E --> H
  G1 --> H
...
*图 2-2：世界模型问题谱系。三条路线回答不同问题并各自进入闭环；谱系位置不决定部署质量，也不构成代际替代关系。来源：本书原创，CC BY-NC 4.0，2026-09-03。*<!-- INTERNAL_ASSET_ID: FIG-02-02 -->

这张谱系图表达的是**问题定义的分叉与汇合**，不是单线的代际替代。较新的生成模型没有淘汰潜在动力学模型；一个能生成高保真视频的系统，也不必然比一个只预测紧凑状态的模型更适合实时控制。

可以把常见工作归纳为六类。它们之间存在交叉，一个系统也可能同时占据多列。

| 技术谱系 | 主要预测对象 | 动作如何进入 | 主要用途 | 首要评价 | 典型代表 |
|---|---|---|---|---|---|
| 潜在动力学 | 紧凑状态、观测与回报 | 显式动作条件 | 规划、想象训练、提高交互效率 | 控制回报、长期误差、数据效率 | World Models、PlaNet、Dreamer |
| 决策等价模型 | 与搜索有关的奖励、价值、策略和隐状态 | 作为树搜索中的转移输入 | 搜索与决策 | 决策表现、搜索效率 | MuZero |
| 预测表征 | 未来或被遮挡区域的抽象表示 | 可无动作，也可后训练为动作条件 | 自监督理解、迁移、规划前端 | 表征质量、预测与迁移能力 | JEPA、V-JEPA、V-JEPA 2 |
| 生成式未来模型 | 像素、视频 token、场景状态或多模态观测 | 动作、文本、轨迹或结构条件 | 未来预测、数据生成、场景编辑 | 可控性、时空一致性、物理合理性 | 视频世界模型、GAIA |
| 可交互环境模型 | 随用户或智能体输入持续演化的观测流 | 键鼠、潜动作或智能体动作 | 生成训练环境、开放式交互 | 响应延迟、长期一致性、可玩性 | Genie 系列 |
| 世界基础模型与平台 | 可适配到具体领域的通用世界表征或生成模型 | 由后训练任务定义 | 合成数据、领域适配、仿真与评估基础设施 | 可适配性、覆盖度、下游闭环收益 | Cosmos 等 |

#### 从历史影响看：改变“接口”的工作比刷新榜单的工作更重要

本书选择历史锚点时，不以参数规模或发布年份为唯一标准，而看它是否改变了人们使用世界模型的方式：

1. **World Models** 将视觉编码器、时序模型和控制器拆开，清楚展示了“在学得的梦境中训练策略”这一现代直觉。它并不是世界模型概念的起点，但成为深度学习时代极具传播力的结构化范例。
2. **PlaNet** 把随机潜在动力学与在线规划紧密连接，突出“为了行动而学习状态”的路线。
3. **Dreamer** 将重心从每一步在线搜索转向在潜在想象轨迹中学习行为，形成持续演化的一条模型式强化学习主线。
4. **MuZero** 表明模型可以不重建环境全部细节，只学习足以支持奖励、价值、策略和搜索的动力学。这使“正确复原世界”与“保留决策所需信息”被明确区分。
5. **JEPA 与 V-JEPA** 将预测目标从像素移到抽象表征，强调舍弃难以预测但与任务无关的细节；动作条件版本才进一步接近可用于闭环规划的环境模型。
6. **Genie、GAIA 与 Cosmos 一类生成式系统** 把世界模型扩展为可交互环境、垂直领域模拟器或可定制的物理 AI 基础设施。此时，“模型”有时指单个预测网络，有时已经指包含数据处理、后训练和评估组件的平台。

这条历史线索揭示了一个关键变化：世界模型的输出先从**任务相关的紧凑状态**扩展到**可感知的高维未来**，用途也从**智能体内部规划**扩展到**训练环境、合成数据和系统验证**。输出变得更可视化，不代表它自动获得了更强的因果性、可控性或决策价值。

#### 为什么不同机构对“世界模型”的理解不同

机构之间的差异通常来自所服务的产品闭环，而不只是研究偏好。

| 机构或研究传统 | 常见的世界模型口径 | 位于闭环中的位置 | 容易产生的误读 |
|---|---|---|---|
| 模型式强化学习社区 | 学习环境动力学，在模型中规划或训练策略 | 策略内部或紧邻策略 | 误以为必须生成清晰视频 |
| Meta 的 JEPA 路线 | 在抽象表征空间预测物理世界，并逐步加入动作与规划 | 感知表征到规划之间 | 把无动作的视频编码器直接当成完整控制模型 |
| Google DeepMind | 同时包含 Dreamer、MuZero 式决策模型和 Genie 式交互环境生成 | 从智能体内部到外部训练环境 | 误以为同一机构的所有“世界模型”都有相同接口 |
| NVIDIA Cosmos | 面向物理 AI 的世界基础模型、数据工具、后训练与评估平台 | 数据生产和模型开发基础设施 | 把整个平台的能力归因于某一个模型权重 |
| Wayve GAIA | 面向驾驶的视频生成、场景控制、训练和安全评估 | 自动驾驶开发与验证链路 | 将视觉逼真度直接等同于闭环驾驶有效性 |
| 机器人策略团队 | 能预测动作后果、辅助规划或提升策略学习的数据模型 | 机器人感知—决策—控制闭环 | 把 VLA 策略本身称作世界模型，忽略显式预测接口 |

因此，“某公司正在做世界模型”至少可能表示四件不同的事：训练一个智能体内部的动力学模块；训练一个通用视频预测器；构造可供智能体交互的生成环境；或者建设服务数据和评估的模型平台。只有明确输入、输出、动作接口和使用位置，这句话才具有可比较的技术含义。

#### 不存在脱离任务的“世界模型总 SOTA”

“SOTA”必须与赛道、数据、协议和用途绑定。把所有世界模型放进一张总排行榜，至少会混淆以下评价目标：

| 赛道 | 更有意义的指标 | 不能由这些指标单独推出什么 |
|---|---|---|
| 潜在控制与模型式强化学习 | 环境回报、样本效率、规划时间、跨任务泛化 | 不能推出生成视频最逼真 |
| 预测表征 | 动作识别、未来预测、迁移、少样本适配 | 不能推出已经具备动作条件闭环能力 |
| 机器人规划 | 未见物体与环境中的任务成功率、重规划稳定性 | 不能推出适用于长时复杂操作 |
| 生成式视频世界 | 感知质量、时空一致性、条件遵循、物理合理性 | 不能推出可安全替代真实环境 |
| 可交互生成环境 | 输入响应、长时一致性、可控性、环境多样性 | 不能推出对真实机器人存在有效迁移 |
| 自动驾驶世界模型 | 多视角一致性、行为可控性、长尾覆盖、仿真与实测相关性 | 不能推出已经满足安全验证要求 |
| 世界基础模型平台 | 领域后训练效率、数据产能、工具兼容性、下游收益 | 不能推出其中每个组件都达到同一能力水平 |

因此，本书使用“领先”“SOTA”或“最强”时，应同时给出五项限定：**对象是什么、在哪个数据集或环境、采用什么指标、与谁比较、结果来自论文报告还是本书测量**。缺少其中任一项时，更稳妥的写法是“代表性工作”“该路线的公开系统”或“在某项公开评测中报告领先结果”。

#### 判断一个新模型是否值得进入正文

面对快速更新的模型，读者可以使用下面的六问，而不必先记住产品代际：

1. **状态问题**：它表示的是像素、几何、对象、语义、潜变量，还是只保留价值相关信息？
2. **转移问题**：它是否真正学习时间变化，还是只做单帧理解、补全或检索？
3. **干预问题**：动作是否作为明确条件进入模型？改变动作时，预测结果是否会系统变化？
4. **用途问题**：预测被用于规划、策略训练、合成数据、评估，还是仅用于可视化展示？
5. **闭环问题**：证据来自离线预测指标、模拟闭环，还是真实系统中的任务结果？
6. **边界问题**：失败来自感知、动力学、长时记忆、动作接口，还是下游规划器？

只有当一项工作改变了上述某个问题的答案，或者提供了新的可验证闭环，它才值得进入正文。仅刷新参数量、分辨率、视频时长或单个榜单成绩的版本，更适合放入研究雷达，而不是反复改写稳定章节。

#### 与相邻概念的最后一道边界

- **视频生成模型不自动等于世界模型**：若条件变化不能对应到可解释的环境干预，生成结果再逼真也可能只是相关性拟合。
- **世界模型不自动等于模拟器**：传统模拟器可能由人工规则构成；学得的世界模型来自数据。两者可以组合，也可以相互蒸馏或校准。
- **VLA 不自动等于世界模型**：VLA 直接输出动作；只有当系统具有可查询的状态转移或结果预测接口时，才应进一步讨论其世界建模部分。
- **数字孪生不自动等于世界模型**：数字孪生强调与特定真实对象或系统同步，世界模型可以是跨对象、跨环境的统计模型。
- **基础模型不自动等于世界模型**：规模和通用性不能替代时间转移、干预响应和闭环用途。

对具身智能而言，最有价值的世界模型未必是“知道最多”或“画得最好”的模型，而是能在给定资源和风险边界内，保留任务所需状态、响应可执行动作、表达关键不确定性，并对最终闭环产生可验证增益的模型。
...
## 2.8 八类系统的四轴模型卡（实验 2-1<!-- INTERNAL_ASSET_ID: EXP-02-01 -->）
...
实验 2-1 v4<!-- INTERNAL_ASSET_ID: EXP-02-01 v4 --> 中，只有 3/8 固定卡片同时满足“学习动态 + 候选动作条件”，而转移证据、动作干预和直接策略输出分别属于不同集合。该计数只描述本书选定的八个教学 archetype，不估计现实项目比例。
...
| 本书结果 | 八类系统具有四轴、三态能力、来源和证据限制记录 | 实验 2-1<!-- INTERNAL_ASSET_ID: EXP-02-01 --> | CPU smoke | 教学元数据分类，不是项目比例或性能 benchmark |
| 本书结果 | current-only 与 history-aware state 在固定等权 context 上有 0.5 decision gap | 实验 2-1<!-- INTERNAL_ASSET_ID: EXP-02-01 --> | CPU smoke | 一步手工 return，不是 learned memory 或真实 POMDP 性能 |
...
以遮挡下移动杯子为例，只补全被挡住的杯子图像是在预测 observation；保存杯子可能位置的分布是在维护 belief；比较“继续伸手、换视角、先停下”会怎样改变接触与后续观测，才进入动作条件的世界模型。施工改道也可作同样区分：生成一段逼真道路视频不等于能够回答不同减速或变道动作的后果。两个案例共用的判据不是画面是否逼真，而是模型表示什么、接受什么动作、怎样推进，以及输出被用于何种决策。
