...
读者应能把一个宽泛方向收缩为可证伪问题，区分现象、测量、机制和部署四类主张，并为每类主张匹配最低充分证据。读者还应能把数据、模型、环境、评测与安全组织成证据图，解释哪些结论已经成立、哪些只是假设，以及何时应缩小主张或停止。
...
S 档 capstone 可以在无 GPU 条件下完整表达研究问题、接口与证据链；M/L1/L2 则保持为可执行但 `pending` 的扩展合同。不得编造训练日志、性能数字或 GPU 资源；明确“未运行”本身就是可靠证据边界。
