...
章节中的微型结果主要用于说明定义和暴露指标盲区，不代表大型模型、真实机器人或车辆性能。更详细的维护状态集中在`maintenance/status.md` 仓库页面，普通阅读无需先理解这些机器合同。

本书采用“公式服务于解释”的轻量数学策略：当公式能消除自然语言对状态、时间、概率或优化目标的歧义时就写出公式；当一串符号只是在重复代码接口或增加记忆负担时，优先使用图、表和反例。公式数量不是深度指标，读者需要掌握的是每个量的语义、假设、作用域及其在闭环中的位置。

## 当前版本与开放出版

当前为 `v0.1.0-rc1` 在线开源发布候选。22 章内容已经形成完整知识链，后续重点是出版编辑、读者测试与引用整理，而不是把所有新模型持续塞进正文。
