...
- **HTML 在线版是规范版本**：内部链接、交叉引用、勘误入口和最新修订以它为准。
- **PDF 是稳定输出**：适合连续阅读、打印、评审和归档；它不拥有独立内容。
- **EPUB 是稳定输出**：适合电子阅读器与可重排阅读；复杂表格和 Mermaid 图应提供文本解释，不能只依赖固定画布。

“规范版本”不等于“唯一值得阅读的版本”。它只说明发生差异时，应回到哪一份源内容裁决。
...
| 稳定版 | 可引用、可归档的编号版本 | 在线候选版条件；PDF/EPUB 人工检查；已处理阻断性试读反馈 |
...
- 目录、章节编号、图表编号和交叉引用一致；
- 公式没有截断，代码块没有溢出，宽表在窄屏或纸面上仍可理解；
- 每幅图即使脱离颜色也能辨认，并有足以传达结论的图注或正文解释；
- 外部事实能追溯到参考文献，作者观察与论文结论没有混写；
- 正文、原创图表与代码资产的许可边界清楚；
- PDF 检查封面、目录、页眉页脚、分页孤行与空白页；
- EPUB 检查目录层级、公式替代呈现、图片缩放和阅读器重排。
