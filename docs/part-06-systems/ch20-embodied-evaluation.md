...
第15章已完成当前审查，第17章已形成正文与 S 档 model-exploitation 实验；第19章尚未成稿，因此仿真接口仍只按最小评测契约引用。当前不能在第17章交叉审查和第19章接口完成前标记 `reviewed`。
...
本书后续默认通用仿真选择为：机器人任务优先轻量、已有官方评测接口的环境；驾驶闭环优先 MetaDrive，CARLA 作为高保真扩展。具体选择将在第19章锁定。它们均不是完成 S 档正文的前提。
...
| 未验证 | 通用仿真上的策略成功与鲁棒性 | 后续 M 档 | planned | 等待第19章环境锁定 |
...
第21章将把评测字段连接到实时性、故障监测和最小风险动作。本章的时延分位数、接管、失败日志和安全定义会成为部署门禁；第19章完成后还需结合第17章的 simulator gap 与 policy exploitation 回做一致性审查。
...
- 内容审查：通过；
- 代码审查：通过；
- 一致性审查：修改中（第15章及第17章草稿接口已核对，等待第19章与批次 D 审查）；
- 教学审查：通过；
