...
第15、17、19章尚未成稿，因此 VLA、世界模型辅助策略和仿真接口在本章中只按最小评测契约引用。批次 B 可形成正文草稿，但不能在这些依赖完成前标记 `reviewed`。
...
第21章将把评测字段连接到实时性、故障监测和最小风险动作。本章的时延分位数、接管、失败日志和安全定义会成为部署门禁；在第15、17、19章完成后还需回做一致性审查。
