...
第15章已成稿并完成当前审查；第17、19章尚未成稿，因此世界模型辅助策略和仿真接口在本章中只按最小评测契约引用。当前可完成内容、代码和教学审查，但不能在这两个依赖完成前标记 `reviewed`。
...
第21章将把评测字段连接到实时性、故障监测和最小风险动作。本章的时延分位数、接管、失败日志和安全定义会成为部署门禁；在第17、19章完成后还需回做一致性审查。
