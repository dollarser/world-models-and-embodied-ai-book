...
一个离线或仿真中有效的模型，怎样才能在有频率、延迟、传感器陈旧、动作边界和故障的系统里执行？平均推理速度为什么不能证明实时性？模型失败时又该由谁选择降级行为？
...
读者应能分解一次控制周期，报告 mean/p95/p99/max 与 deadline miss，检查观测和 action chunk 新鲜度，设计有原因码的执行网关，并为具体本体写出降级与恢复合同。
...
*FIG-21-01：部署控制周期与独立网关。来源：本书原创，MIT，2026-08-31。fallback 是接口而非固定命令。*

每个动作 packet 至少携带：输入时间戳、生成时间、适用起始步、有效截止步、控制频率、单位/frame、归一化版本、动作范围、模型/checkpoint 和 trace ID。网关不需要理解语言，却必须拒绝旧观测、超时、NaN/Inf、越界、过期 chunk 和版本不兼容。
...
固定延迟为 `20, 22, 24, 26, 28, 150 ms`，deadline 为 50 ms。另用六个 packet 分别覆盖健康、旧观测、超时、非有限动作、越界和过期 chunk。
...
*TAB-21-02：六个固定 packet 的网关原因码。fallback 标签不是执行器命令。*

`CLAIM-21-03`（result）：六个 packet 中只有健康包通过，五种注入分别产生唯一原因码并进入 fallback。该结果验证网关实现，不估计真实系统故障率或安全性。

结果保存在 `results/ch21/EXP-21-01-smoke.json`；7 个单元测试还拒绝非法 config、非有限 latency 和错误 percentile。
...
部署把模型问题变成时序和系统问题。必须测端到端年龄、尾延迟和 deadline miss，异步 action chunk 还要管理队列与新鲜度。独立网关拒绝旧、迟、非法和过期动作；降级模式由具体本体和场景定义，而不是一个万能零向量。
...
- 一致性审查：修改中；
- 教学审查：修改中；
- 已知限制：没有测量真实墙钟、调度器、网络、模型、ROS、机器人、车辆或 GPU，也不构成安全认证。
