...
读者应能画出 JEPA 训练数据流，区分像素、全局和 dense 表征，设计带 nuisance shift、时间反转与动作反事实负对照的 probe，并为官方 V-JEPA 2.x 推理编写资源与许可预检清单。
...
视频 JEPA 把 mask 扩展到时间和空间，训练目标要求预测时空目标表示，但这不自动证明某个下游读出保留了时间方向。V-JEPA 的特征预测仍不等同未来像素生成：它可以在 latent 中预测变化，而不给出可观看视频，也可能忽略表示空间没有编码的细节。必须用同帧集合的原序/反序配对或未来预测任务，区分静态内容捷径与时间箭头；只读出单帧对象或中间状态不是时序证据。
...
第三个诊断构造八条三步标量序列。每个中间状态都分别配一条正向序列 $c-1,c,c+1$ 和反向序列 $c+1,c,c-1$，因此方向标签平衡，且一对序列共享同一中间状态。`middle_frame` 只暴露 $c$，`ordered_delta` 额外暴露 $x_{last}-x_{first}$。两个接口的当前状态读出都为零误差，但只有后者能区分时间方向。

| temporal 接口 | 当前状态 probe RMSE ↓ | 方向 accuracy ↑ | 反转敏感度 ↑ |
| --- | ---: | ---: | ---: |
...
JEPA 把预测目标从像素移到表示空间，使模型可以忽略部分不可预测细节，但“忽略什么”必须由下游 probe、扰动和闭环用途检查。重建差不等于任务差，当前状态可读也不等于保留时间箭头，probe 好更不等于规划正确。可靠协议需要负对照、分布偏移、时间变换、统一 head 与明确资源/许可边界。
...
6. **指标设计**：解释为什么反转敏感度非零仍不足以证明模型理解物理时间，并给出一个配对负对照。
