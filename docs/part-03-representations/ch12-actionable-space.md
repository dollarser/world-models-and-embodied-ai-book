...
> 资料核查日期：2026-08-31
> 关联实验：`EXP-12-01`
> 关联声明：`CLAIM-12-01`～`CLAIM-12-06`
> 关联图表：`FIG-12-01` / `FIG-12-02` / `TAB-12-01` / `TAB-12-02`
> 资源档位：S / M / L1 / L2
> GPU 状态：待验证
...
对规划 horizon `H`，一种保守碰撞查询是检查 footprint 在每个未来时刻是否与 occupied 或风险未知区域相交。这里的保守不是永远停止：系统可以主动换视角、减速、请求新观测或选择已知通道，但必须把探索行为和风险预算写进规划器。
...
S 档实验在 `7×7` BEV 上从固定原点发出三条无噪声深度射线：射线内部标 free，三个回波端点标 occupied，其余保持 unknown。实验再计算从原点可达的 free 区、障碍旁接近位置、占用格的一格坐标偏移，以及一个动态目标更新。
...
S 档 `EXP-12-01` 使用 Python 标准库、CPU、零下载和 MIT 程序化 fixture；它只验证三态地图、连通、坐标误差与动态更新。
...
| 类型 | 声明/结果 | 来源 | 状态 | 限制 |
| --- | --- | --- | --- | --- |
| 本书结果 | 三态射线、坐标偏移与动态路径 smoke | `EXP-12-01` | CPU smoke | 2D 无噪声手工格子 |
...
3. **时间实验**：延迟一帧更新动态障碍，记录路径判定在哪个时刻改变。
...
- 下一步：在真实 RGB-D/驾驶数据上加入标定噪声与动态 occupancy；当前接口已与第3、10、11、15、19章核对。
