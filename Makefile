...
check-local:
	...
	python3 scripts/validate_experiment_card.py labs/track-a-world-model-control/ch08-imagination-learning/experiment-card.json
	...
	python3 scripts/validate_experiment_card.py labs/track-b-policy/ch18-post-training/experiment-card.json
	...
	python3 -m unittest discover -s tests -p 'test_serve_book.py'
...
ch01-smoke-local:
	python3 labs/track-a-world-model-control/ch01-closed-loop/scripts/smoke.py

ch01-test-local:
	python3 -m unittest discover -s labs/track-a-world-model-control/ch01-closed-loop/tests -p 'test_*.py'
...
ch08-smoke:
	python3 scripts/docker_compose.py run --rm ch08-smoke

ch08-smoke-local:
	python3 labs/track-a-world-model-control/ch08-imagination-learning/scripts/smoke.py

ch08-test-local:
	python3 -m unittest discover -s labs/track-a-world-model-control/ch08-imagination-learning/tests -p 'test_*.py'
...
ch18-smoke:
	python3 scripts/docker_compose.py run --rm ch18-smoke

ch18-smoke-local:
	python3 labs/track-b-policy/ch18-post-training/scripts/smoke.py

ch18-test-local:
	python3 -m unittest discover -s labs/track-b-policy/ch18-post-training/tests -p 'test_*.py'
...
ch22-smoke:
	python3 scripts/docker_compose.py run --rm ch22-smoke

ch22-smoke-local:
	python3 labs/track-d-capstone/ch22-project-audit/scripts/smoke.py

ch22-test-local:
	python3 -m unittest discover -s labs/track-d-capstone/ch22-project-audit/tests -p 'test_*.py'
