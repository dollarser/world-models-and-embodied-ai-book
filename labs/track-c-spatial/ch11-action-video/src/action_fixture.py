...
def evaluate_model(model: str) -> dict[str, float]:
    ...
    for actions in sequences:
        expected_endpoint = rollout((1.0, 3.0), actions)[-1]
        predicted_endpoint = rollout((1.0, 3.0), actions, model)[-1]
        endpoint_errors.append(hypot(predicted_endpoint[0] - expected_endpoint[0], predicted_endpoint[1] - expected_endpoint[1]))
    ...
    return {
        ...
        "action_sensitivity": len(set(predicted_counterfactuals)) / len(ACTIONS),
...
def evaluate() -> dict[str, dict[str, float]]:
    return {model: evaluate_model(model) for model in ("action_blind", "action_conditioned")}
