...
| 命令 | 环境 | 检查范围 | 失败等级 |
| --- | --- | --- | --- |
| `make check-local` | 宿主标准库 | 文件、JSON、链接、manifest、声明/图表双向登记、`fact`/`inference` 证据登记、关键 `recommendation` 适用性登记、Mermaid 无障碍元数据、标题层级、章节教学/接口区段、读者关键术语、`result`—实验卡绑定/定义句边界与 22 组 smoke—结果精确一致性 | `BLOCK` |
